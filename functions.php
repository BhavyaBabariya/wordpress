...
function custom_post_field_meta_box() {
    add_meta_box(
        'custom_post_field',          // ID of the meta box
        'Custom Post Field',          // Title of the meta box
        'custom_post_field_callback', // Callback function
        'post',                       // Screen where it will appear (post, page, or custom post type)
        'normal',                     // Context (normal, side, or advanced)
        'high'                        // Priority (high, low, or default)
    );
}
add_action('add_meta_boxes', 'custom_post_field_meta_box');

function custom_post_field_callback($post) {
    wp_nonce_field('save_custom_post_field', 'custom_post_field_nonce');
    $custom_value = get_post_meta($post->ID, '_custom_post_field_key', true);
    ?>
    <label for="custom_post_field">Enter your custom field value:</label>
